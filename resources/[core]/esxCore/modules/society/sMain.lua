local Society = require "modules.society.cfgSociety"
local Locale_Society = Society.Locale_Society
...
AddEventHandler('esx_society:checkSocietyBalance', function(society)
	...
	TriggerEvent('esx_addonaccount:getSharedAccount', society.account, function(account)
        TriggerClientEvent('esx:showNotification', xPlayer.source, Locale_Society['check_balance'] .. ESX.Math.GroupDigits(account.money) .. ' $')
	end)
end)
...
AddEventHandler('esx_society:withdrawMoney', function(societyName, amount)
	...
	TriggerEvent('esx_addonaccount:getSharedAccount', society.account, function(account)
		if amount > 0 and account.money >= amount then
			...
			xPlayer.addMoney(amount, Locale_Society['money_add_reason'])
            TriggerClientEvent('esx:showNotification', xPlayer.source, Locale_Society['have_withdrawn'], ESX.Math.GroupDigits(amount) .. ' $')
		else
            TriggerClientEvent('esx:showNotification', xPlayer.source, Locale_Society['invalid_amount'])
		end
	end)
end)
...
AddEventHandler('esx_society:depositMoney', function(societyName, amount)
	...
	if amount > 0 and xPlayer.getMoney() >= amount then
		TriggerEvent('esx_addonaccount:getSharedAccount', society.account, function(account)
			xPlayer.removeMoney(amount, Locale_Society['money_remove_reason'])
            TriggerClientEvent('esx:showNotification', xPlayer.source, Locale_Society['have_deposited'], ESX.Math.GroupDigits(amount) .. ' $')
...
RegisterNetEvent('esx_society:setJobLabel')
AddEventHandler('esx_society:setJobLabel', function(job, grade, label)
    local source = source
    local xPlayer = ESX.GetPlayerFromId(source)
	local playerName = xPlayer.getName()
	local gradeName = Jobs[job].grades[tostring(grade)].label
...
RegisterNetEvent('esx_society:setJobSalary')
AddEventHandler('esx_society:setJobSalary', function(job, grade, salary)
    local source = source
    local xPlayer = ESX.GetPlayerFromId(source)
	local playerName = xPlayer.getName()
	local gradeName = Jobs[job].grades[tostring(grade)].label
	...
	if xPlayer.job.name == job and xPlayer.job.grade_name == 'boss' then
		if salary <= Config.MaxSalary then
            ...
            TriggerClientEvent('esx:showNotification', xPlayer.source, Locale_Society['invalid_amount_max'], Config.MaxSalary .. ' $')
        end
    end
end)
	...
	function(result)
		for k, row in pairs(result) do
			...
			if not alreadyInTable then
				local name = 'name not found'--TranslateCap('name_not_found')
