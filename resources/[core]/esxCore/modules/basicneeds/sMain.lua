local Basicneeds = require 'modules.basicneeds.cfgBasicneeds'
...
local function handleItemUsage(itemName, itemConfig, source)
    ...
    if itemConfig.type == "food" then
        statusType = "hunger"
        notificationMessage = Basicneeds.Locale_Basicneeds['used_food'].. ESX.GetItemLabel(itemName)
    elseif itemConfig.type == "drink" then
        statusType = "thirst"
        notificationMessage = Basicneeds.Locale_Basicneeds['used_drink'].. ESX.GetItemLabel(itemName)
    else
...
CreateThread(function()
    for itemName, itemConfig in pairs(Basicneeds.Basicneeds.Items) do
...
ESX.RegisterCommand('heal', 'admin', function(xPlayer, args, showError)
    ...
    args.playerId.showNotification(Basicneeds.Locale_Basicneeds['got_healed'])
end, true, {
