...
local function billPlayerByIdentifier(targetIdentifier, senderIdentifier, sharedAccountName, label, amount)
	...
	if string.match(sharedAccountName, "society_") then
		return TriggerEvent('esx_addonaccount:getSharedAccount', sharedAccountName, function(account)
			...
			xTarget.showNotification(Locale_Billing['received_invoice'])
		end)
	end
...
ESX.RegisterServerCallback('esx_billing:payBill', function(source, cb, billId)
	...
	if result.target_type == 'player' then
		if not xTarget then
			xPlayer.showNotification(Locale_Billing['player_not_online'])
			return cb()
		end
		...
		if xPlayer.getMoney() < amount then
			...
			if xPlayer.getAccount('bank').money < amount then
				xTarget.showNotification(Locale_Billing['target_no_money'])
				xPlayer.showNotification(Locale_Billing['no_money'])
				return cb()
			end
		end
		...
		xPlayer.showNotification(Locale_Billing['paid_invoice'].. groupedDigits)
		xTarget.showNotification(Locale_Billing['received_payment'].. groupedDigits)
