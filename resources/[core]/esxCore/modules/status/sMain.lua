...
RegisterNetEvent('esx_status:update', function(status)
    ...
    if status.hunger and status.hunger.val < 1000 then
        xPlayer.showNotification('Vous avez faim !', 'info')
	elseif status.thirst and status.thirst.val < 1000 then
		xPlayer.showNotification('Vous avez soif !', 'info')
	elseif status.health and status.health.val < 1000 then
		xPlayer.showNotification('Vous avez besoin de soins !', 'info')
    end
end)