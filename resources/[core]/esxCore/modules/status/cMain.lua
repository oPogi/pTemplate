	...
	for i=1, #Status, 1 do
		if minimal then
			table.insert(status, {
				name    = Status[i].name,
				val     = Status[i].val,
				percent = (Status[i].val / Status.Status.StatusMax) * 100
			})
		else
			table.insert(status, {
				name    = Status[i].name,
...
RegisterNetEvent('esx_status:load', function(status)
	...
	if Status.Status.Display then TriggerEvent('esx_status:setDisplay', 0.5) end
...
CreateThread(function()
	while true do
		Wait(Status.Status.UpdateInterval)
